...
import { applyPracticeOutcome, getCorrectAnswer, isCorrectAnswer, prettyStepLabel } from "../../core/practice";
import {
...
export function PracticeScreen() {
  ...
  function handleAnswer(option: string) {
    if (!currentCase || !currentStep || currentResult) return;
  ...
  async function finishCase() {
    ...
    const elapsedSeconds = state.sessionState.caseStartMs
      ...
      : 0;
    const stepResults = state.sessionState.stepResults.filter((result): result is StepResult => Boolean(result));
    const seenCasesByDifficulty = state.storage?.loadSeenCaseState() ?? createEmptySeenCasesState();
    ...
    await state.storage?.saveAttempt({
      user_id: state.userId ?? null,
      case_id: currentCase.case_id ?? null,
      difficulty: getDifficultyLabel(payload?.progressionConfig ?? null, Number(currentCase.difficulty_level ?? 1)),
      difficulty_level: Number(currentCase.difficulty_level ?? 1),
      xp_earned: outcome.summary.totalXpAward,
      correct: outcome.summary.correctSteps,
      total_questions: outcome.summary.totalSteps,
      time_taken_ms: Math.max(0, Math.round(elapsedSeconds * 1000)),
